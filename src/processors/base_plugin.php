...
if ( !class_exists( 'ICWP_Maid_Processor_BasePlugin', false ) ):

	require_once( dirname( __FILE__ ) . ICWP_DS . 'base_maid.php' );

	class ICWP_Maid_Processor_BasePlugin extends ICWP_Maid_Processor_BaseMaid {
		...
		public function addNotice_rate_plugin( $aNoticeAttributes ) {
			...
			$aRenderData = array(
				...
				'strings' => array(
					'dismiss' => _wpmaid__( "I'd rather not show this support" ).' / '._wpmaid__( "I've done this already" ).' :D',
		...
		protected function addNotice_php53_version_warning( $aNoticeAttributes ) {
			...
			$aRenderData = array(
				'notice_attributes' => $aNoticeAttributes,
				'strings' => array(
					'your_php_version' => sprintf( _wpmaid__( 'Your PHP version is very (10+ years) old: %s' ), $oDp->getPhpVersion() ),
					'future_versions_not_supported' => sprintf( _wpmaid__( 'Future versions of the %s plugin will not support your PHP version.' ), $oCon->getHumanName() ),
					'ask_host_to_upgrade' => sprintf( _wpmaid__( 'You should ask your host to upgrade or provide a much newer PHP version.' ), $oCon->getHumanName() ),
					'any_questions' => sprintf( _wpmaid__( 'If you have any questions, please leave us a message in the forums.' ), $oCon->getHumanName() ),
					'dismiss' => _wpmaid__( 'Dismiss this notice' ),
					'forums' => __( 'Support Forums' )
				),
				'hrefs' => array(
					'forums' => 'https://wordpress.org/support/plugin/wp-simple-firewall',
		...
		protected function addNotice_plugin_update_available( $aNoticeAttributes ) {
			...
			if ( !$oWp->getIsPage_Updates() && $oWp->getIsPluginUpdateAvailable( $sBaseFile ) ) { // Don't show on the update page
				...
				$aRenderData = array(
					...
					'strings' => array(
						'plugin_update_available' => sprintf( _wpmaid__( 'There is an update available for the "%s" plugin.' ), $this->getController()->getHumanName() ),
						'click_update' => _wpmaid__( 'Please click to update immediately' ),
						'dismiss' => _wpmaid__( 'Dismiss this notice' )
					),
					'hrefs' => array(
		...
		protected function addNotice_translate_plugin( $aNoticeAttributes ) {
			...
			if ( $this->getIfShowAdminNotices() ) {
				$aRenderData = array(
					'notice_attributes' => $aNoticeAttributes,
					'strings' => array(
						'like_to_help' => sprintf( _wpmaid__( "Would you like to help translate the %s plugin into your language?" ), $this->getController()->getHumanName() ),
						'head_over_to' => sprintf( _wpmaid__( 'Head over to: %s' ), '' ),
		...
		protected function addNotice_post_plugin_update( $aNoticeAttributes ) {
			...
			if ( $this->getInstallationDays() <= 1 ) {
				$sMessage = sprintf(
					_wpmaid__( "Notice - %s" ),
					sprintf( _wpmaid__( "The %s plugin does not automatically turn on certain features when you install." ), $sHumanName )
				);
			}
			else {
				$sMessage = sprintf(
					_wpmaid__( "Notice - %s" ),
					sprintf( _wpmaid__( "The %s plugin has been recently upgraded, but please remember that new features may not be automatically enabled." ), $sHumanName )
				);
			}
			...
			$aRenderData = array(
				...
				'strings' => array(
					...
					'read_homepage' => _wpmaid__( 'Click to read about any important updates from the plugin home page.' ),
